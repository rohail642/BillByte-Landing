  ...
  return (
    <motion.div ref={ref}
      ...
      <div className="tilt-card" style={{
        ...
        position: 'relative',
        transform: `rotate(${plan.tilt}deg)`,
        boxShadow: plan.highlight ? '0 12px 48px rgba(0,0,0,0.18)' : '0 2px 8px rgba(0,0,0,0.04)',
      }}
      onMouseEnter={e => { e.currentTarget.style.transform = 'rotate(0deg) translateY(-6px)'; e.currentTarget.style.boxShadow = plan.highlight ? '0 20px 56px rgba(0,0,0,0.24)' : '0 14px 36px rgba(0,0,0,0.1)' }}
      onMouseLeave={e => { e.currentTarget.style.transform = `rotate(${plan.tilt}deg)`; e.currentTarget.style.boxShadow = plan.highlight ? '0 12px 48px rgba(0,0,0,0.18)' : '0 2px 8px rgba(0,0,0,0.04)' }}>
        {plan.badge && (
          <div style={{
            position: 'absolute', top: -14, left: '50%',
            transform: 'translateX(-50%) rotate(-3deg)',
            background: 'var(--green)', color: 'white', fontSize: 10, fontWeight: 700,
            padding: '4px 14px', borderRadius: 6, fontFamily: 'Outfit',
        ...
        <div style={{ borderTop: `1.5px dashed ${plan.highlight ? 'rgba(255,255,255,0.18)' : 'var(--border2)'}`, paddingTop: 20, marginBottom: 24 }}>
          {plan.features.map(f => (
        ...
        <a href={plan.href} target={plan.href.startsWith('mailto') ? '_self' : '_blank'} rel="noreferrer"
          ...
          onMouseEnter={e => {
            if (plan.highlight) { e.currentTarget.style.background = 'var(--green2)' }
            else { e.currentTarget.style.borderColor = 'var(--text)'; e.currentTarget.style.color = 'var(--text)' }
          }}
          onMouseLeave={e => {
            if (plan.highlight) { e.currentTarget.style.background = 'var(--green)' }
      ...
      <div style={{ maxWidth: 1040, margin: '0 auto' }}>
        <motion.div ref={ref}
          ...
          <h2 style={{ fontSize: 'clamp(30px, 4vw, 52px)', fontWeight: 900, letterSpacing: '-2px', color: 'var(--text)', marginBottom: 12 }}>
            Simple, <Scribbled><Em>transparent</Em></Scribbled> pricing.
          </h2>
